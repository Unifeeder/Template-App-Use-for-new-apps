...
**Open it before designing or building any new screen.** Copy patterns from it instead of inventing. The page is single-file with a strict import contract — only `react` and `@/components/ui/*` (icons are inline SVG, toasts are local state) — so it's portable across every app the daily sync touches with zero extra dependencies. 16 sections: Color, Typography, Spacing, Radius, Shadow, Buttons, Forms, Cards, Tables, Filters & Sort, Navigation, Feedback, Overlays, Data Display, Page Layouts, UX Patterns.
